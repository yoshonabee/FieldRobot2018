                   ...
                   float hier_thresh, int dont_show, int ext_output, int save_labels)
{
	...
	detection_with_class* sds = get_actual_detections(dets, nboxes, thresh, &selected_detections_num);

	for (int target = 0; target < selected_detections_num; target++) {
		printf("Fuck you opencv classes:%d, x:%f, y:%f, w:%f, h:%f\n", sds[target].best_class, sds[target].det.bbox.x, sds[target].det.bbox.y, sds[target].det.bbox.w, sds[target].det.bbox.h);		
		}
	//self modified
